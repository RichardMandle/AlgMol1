...
'''
https://github.com/aspuru-guzik-group/stoned-selfies/blob/main/sim_search.py
https://chemrxiv.org/engage/chemrxiv/article-details/60c753f00f50db6830397c37

Functions for implementing the STONED-SELFIES generative algorithms taken/adapted from above
...
def mutate_selfie(selfie, max_molecules_len, write_fail_cases=False):
    ...
    while not valid:
        ...
        #'Frags' is a used to build our own custom alphabet. First, add possibilities of branches, ring changes etc:
        Frags = ([a + str(n) for n in range(1,10) for a in ['Branch','=Branch','#Branch','=Ring','Ring']])
        
        # Next, add element modifiers and also bonding:
        Frags += ['B','=B','C(F)','=C(F)','C','=C','#C','Si','Ge','N','=N','#N','P','=P','#P','O','=O','S','=S','Se','=Se','F','Cl','Br','I']
        
        # Finally, add brackets to Frags so it's formatted correctly to use as a selfies alphabet:
        alphabet = ['[' + a + ']' for a in Frags]
...
def StonedEngine(smi,method='ECFP4',samples=250,silent=False):
        
    total_time = time.time()
    num_random_samples = samples     
    ...
    if not silent:
        ...
        fig, ax = plt.subplots()
        ax.hist(canon_smi_ls_scores,bins=np.arange(0,1,0.05), facecolor='#2ab0ff', edgecolor='k', linewidth=1.5, alpha=0.7)

     
        plt.ylabel('Count')
        plt.xlabel('Score')
        plt.title('Generative Molecules: Scores')
